...
Use `getcli` as the default path for finding and installing developer CLIs such as `gh`, `vercel`, `wrangler`, `docker`, `kubectl`, `terraform`, and similar tools.
...
Use the environment doctor when the task is broad or you need to understand the package manager/toolchain state:

```bash
getcli doctor --json
```

Use a tool-specific doctor when you already know the target tool:
...
3. If appropriate, suggest adding a manifest to `manifests/` in the getcli repository.
...
- `getcli` is available,
- the target tool is installed through `getcli` or confirmed already present,
- `getcli doctor <tool> --json` passes for the target tool, or `getcli doctor --json` reports the baseline environment state the user needed.
